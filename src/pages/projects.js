...
const FeaturedProject = ({ type = [], title, summary, image, link, github }) => {
  return (
    <article className="w-full flex items-center justify-between relative rounded-br-2xl rounded-3xl border border-solid border-dark bg-light shadow-2xl p-12">
      ...
      >
        <Image src={image} alt={title} className="w-full h-auto" width={1000} height={1000} />
      </Link>
      <div className="w-1/2 flex flex-col items-start justify-between pl-6">
        <span className="text-primary font-medium text-xl">{type.toString().split(',').map((text) => text.toUpperCase()).toString().split(',').join(', ')}</span>
        <Link
          href={link}
          target="_blank"
        ...
        <p className="my-2 font-medius text-dark">{summary}</p>
...
const Project = ({ type, title, image, link, github }) => {
  return (
    <article className="w-full flex flex-col items-center justify-center rounded-2xl border border-solid border-dark bg-light p-6 relative">
      <div className="absolute top-0 -right-3 -z-10 w-[102%] h-[103%] rounded-[2.1rem] bg-dark rounded-br-3xl" />
      <Link
        href={link}
        target="_blank"
